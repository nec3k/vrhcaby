...
class Hra:
    ...
    def spusteni_hry(self, verbose = True):
        ...
        while True:
            ...
            def provedeni_tah(pozice, hodnota_kostky):
                hodnota_pro_provedeni_tah = self.ziskani_hodu_kostkou_pro_provedeni_tahu(pozice, hodnota_kostky, hod_kostkami)
                if hodnota_pro_provedeni_tah is None:
                    raise Provedeni_neplatneho_tahu("Nelze posunout ten kámen o %d" % hodnota_kostky)
                for hodnota in hodnota_kostky:
                    ...
                    tahy.append({'puvodni_pozice': puvodni_pozice, 'hodnota_pro_provedeni_tah': hodnota, 'nove_pozice': pozice})
                    puvodni_pozice.append(hodnota)
            ...
            self.strategie[barva].tah(
                ...
                barva,
                hod_kostkami.copy(),
                lambda pozice, hodnota_kostky: provedeni_tah(pozice, hodnota_kostky),
            ...
            if verbose and len(hod_kostkami) > 0:
                print('Nebyly provedeny všechny tahy. %s hraje %s neposune %s' % (
                    ...
                    hod_kostkami))
                self.hraci_deska.vytiknuti_hraci_desky()
                state = {
    ...
    def ziskani_hodu_kostkou_pro_provedeni_tahu(self, pozice, pozadovany_tah, dostupny_hod):
        if dostupny_hod.__contain__(pozadovany_tah):
            if self.hraci_deska.muze_posunout(self.hraci_deska.vybrani_kamene_v(pozice), pozadovany_tah):
        ...
        if not hraci_deska.muze_posunout(hraci_deska.vybrani_kamene_v(soucasna_pozice), dostupny_hod[0]):
            dostupny_hod = dostupny_hod.kopie()
            dostupny_hod.zpet()
        
        for hodnota in dostupny_hod:
            kamen = hraci_deska.vybrani_kamene_v(soucasna_pozice)
